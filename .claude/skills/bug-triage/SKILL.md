...
- **Target repo:** Always file on the **current repo** (`ercs-second-brain/agent-orchestrator`), not forks.
...
**Always trace the actual code** — don't surface-level diagnose. [#1129](https://github.com/ercs-second-brain/agent-orchestrator/issues/1129) looked like a simple `ao stop` issue but was actually a session lineage/cascade problem.
...
```
Example: [#1391](https://github.com/ercs-second-brain/agent-orchestrator/issues/1391) traced a mobile layout break to a `display: flex` → `display: grid` change.
...
**⛔ NEVER use placeholder URLs.** Upload BEFORE creating the issue. ([#1151](https://github.com/ercs-second-brain/agent-orchestrator/issues/1151) RCA on this pattern.)
...
Example: [PR #1608](https://github.com/ercs-second-brain/agent-orchestrator/pull/1608) was diagnosed High as xterm v6 issue — real cause was a `=` prefix on tmux `set-option`. Should have been Medium.
...
Example: [PR #1608](https://github.com/ercs-second-brain/agent-orchestrator/pull/1608) — source analysis led to wrong theories, npm diff showed the only change was a `=` prefix on tmux `set-option`.
...
- **New fields on shared TS interfaces MUST be optional** (`field?: Type`). Downstream `Partial<X>` spreads break on required fields. Example: [PR #1523](https://github.com/ercs-second-brain/agent-orchestrator/pull/1523).
