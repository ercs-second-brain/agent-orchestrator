...
func TestInstall_WritesSkillAndIsIdempotent(t *testing.T) {
	...
	for _, removed := range []string{"preview.md", "browser.md"} {
		if _, err := os.Stat(filepath.Join(Dir(dataDir), "commands", removed)); !os.IsNotExist(err) {
			t.Fatalf("commands/%s survived removal (err=%v)", removed, err)
		}
	}
