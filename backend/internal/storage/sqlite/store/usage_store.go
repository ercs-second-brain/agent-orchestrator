...
func (s *Store) ApplyUsageChunk(
	...
	err := s.inTx(ctx, "apply usage chunk", func(q *gen.Queries) error {
		...
		for _, ev := range events {
			...
			if err == nil {
				if !usageEventReplayMatches(existing, ev) {
...
func usageEventInsertParams(source gen.GetUsageSourceWithBindingAndSessionRow, ev domain.ModelUsageEvent) gen.InsertModelUsageEventParams {
	// The retired cost columns stay NULL: usage pricing was removed, but stored
	// historical values are left untouched in existing rows.
...
// stringOrNull maps the empty string to SQL NULL.
...
// usageEventReplayMatches reports whether a replayed event is the same durable
// fact as the stored row it deduplicates against. A stored row with no billing
// attribution predates that retired feature and stays token-only comparable so
// replay never rejects it.
func usageEventReplayMatches(existing gen.GetModelUsageEventByKeyRow, event domain.ModelUsageEvent) bool {
	matches := existing.ProviderID == string(event.ProviderID) && existing.ModelID == event.ModelID &&
	...
	if matches && existing.ProviderUsageJson.Valid && event.ProviderUsageJSON != "" {
		matches = existing.ProviderUsageJson.String == event.ProviderUsageJSON
	}
	return matches
}
...
func validateUsageEvent(harness domain.AgentHarness, event domain.ModelUsageEvent) error {
	...
	// the schema comment describes is enforced here.
