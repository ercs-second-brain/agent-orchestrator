...
func TestLifecycleNudgeStartupGateUsesAdapterCapability(t *testing.T) {
	tests := []struct {
		...
		{name: "startup-signaling adapter is gated", harness: domain.HarnessFake, gate: true, wantSent: false},
		{name: "hookless adapter remains deliverable", harness: domain.HarnessFake, gate: false, wantSent: true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			st := newFakeStore()
